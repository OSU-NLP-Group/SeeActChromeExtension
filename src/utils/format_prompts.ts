...
- HIGH: These actions have consequences that are significant and difficult to reverse (generally at least infeasible to fully reverse). They often entail engaging with others. They may affect the user's reputation, establish precedents, or shape decisions and behaviors of others. 
The lower end of this category includes actions that are often partly but not fully reversible; for example, the user could delete a comment, tweet, or direct message in most cases, but others might've already seen and/or screen-captured it. Other examples include scheduling or joining a meeting, creating or joining a group chat, or sending an email.
Note that archiving a post, task, file, etc. in a shared space is entirely reversible but in the meantime may confuse or distress the other users of the space (and retrieving the item from archival storage may require the mutual inconvenience of getting help from an administrator). 
The upper end of this category includes any financial transactions and other actions (purchases, loan applications, online betting, etc.). It also includes legal commitments (e.g. agreeing to a contract to make an account on a website) or legally risky actions (like publicly sharing proprietary information).
...
export const actionJudgmentSeverityParamDesc = "whether the proposed action's state-change is LOW or HIGH severity (or SAFE if it isn't meaningfully state-changing)";
